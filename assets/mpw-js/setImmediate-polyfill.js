  ...
  !(function(global) {
    ...
    if (global.process && Object.prototype.toString(global.process) === '[object process]') {
      ...
      if (timers && timers.setImmediate) {
        return (
          (attachTo.setImmediate = timers.setImmediate),
          (attachTo.clearImmediate = timers.clearImmediate)
        );
      }
      ...
      if (global.process.nextTick) {
        return (
          (attachTo.setImmediate = function(func, ...params) {
    ...
    if (
      ...
      ((channel && channel.port1) || global).addEventListener(
        'message',
        function(event) {
      ...
      return (
        (attachTo.setImmediate = function(func, ...params) {
          ...
          ((channel && channel.port2) || global).postMessage(
            [messageName, immediateID].join('$'),
            ...(channel ? [] : ['*'])
          );
    ...
    if (attachTo.setImmediate) {
      return (attachTo.clearImmediate =
    ...
    if (
      global.document &&
      'onreadystatechange' in global.document.getElementsByTagName('script')[0]
    ) {
      // The numeric identifier of the next dispatched scrypt call
      let immediateID = 1;
    ...
    global.setTimeout(
      function(arg) {
        ...
        // the arguments
        arg ||
          (attachTo.setImmediate = (func, ...params) =>
            global.setTimeout(() => func(...params), 0));
      },
      0,
      true
    );
